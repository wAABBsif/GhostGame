...
void object_update(void)
{
	...
	{
		if (s_objects[i]->type & OBJECT_TYPE_DELETION_BIT)
			continue;

		if (OBJECT_FUNCS[s_objects[i]->type].update != NULL)
			OBJECT_FUNCS[s_objects[i]->type].update(s_objects[i]);
	}
	...
	for (int16_t i = 0; i < s_object_count; i++)
	{
		if (s_objects[i]->type & OBJECT_TYPE_DELETION_BIT)
		{
			OBJECT_FUNCS[s_objects[i]->type & OBJECT_TYPE_DELETION_MASK].end(s_objects[i]);
...
object_id object_get_index(const void *ptr)
{
...
object_id object_add(const size_t size, const object_type type)
{
	...
	s_objects[s_object_count]->type = type;
	s_objects[s_object_count]->id = s_object_current_id;
	OBJECT_FUNCS[s_objects[s_object_count]->type & OBJECT_TYPE_DELETION_MASK].start(s_objects[s_object_count]);

	s_object_count++;
	s_object_current_id++;

	return s_objects[s_object_count - 1]->id;
}

bool object_destroy(const object_id object)
{
	s_objects[object]->type |= OBJECT_TYPE_DELETION_BIT;
	return true;
}